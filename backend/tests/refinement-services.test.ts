...
test('reminder service lists due tasks', async () => {
  const svc = new ReminderService();
  await svc.enqueue({ familyId: 'f1', reason: 'pending commitment', dueAt: new Date('2026-01-01T00:00:00Z') });
  assert.equal((await svc.listPending(new Date('2026-01-02T00:00:00Z'))).length, 1);
});
