...
interface MemoryPrismaLike {
  ...
  reminderTask: MemoryTable;
  overpaymentCredit: MemoryTable;
  familySplitMerge: MemoryTable;
  eventSettlement: MemoryTable;
...
function createMemoryPrisma(): MemoryPrismaLike {
  return {
    ...
    reminderTask: new MemoryTable(),
    overpaymentCredit: new MemoryTable(),
    familySplitMerge: new MemoryTable(),
    eventSettlement: new MemoryTable(),
    async $transaction<T>(cb: TxCallback<T>) { return cb(this as PrismaLikeClient); }
  };
}

const useMemory = env.USE_REAL_DB !== 'true';

let prismaClient: PrismaLikeClient;
if (env.NODE_ENV === 'production' && useMemory) {
  throw new Error('Production requires real database (USE_REAL_DB=true)');
} else if (useMemory) {
